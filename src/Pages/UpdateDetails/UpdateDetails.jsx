...
const UpdateDetails = () => {
  ...
  const onSubmit = async ( data) => {
    ...
    if(res?.data?.success){
      ...
      const articleRes = await AxiosSecure.put(`/allarticles/update/${newUpdatedInfo._id}` , AddArticles)
      console.log(articleRes?.data)
      if(articleRes?.data.modifiedCount > 0){
        Swal.fire({
          
          icon: "success",
          title: "Your Article Update Completed , Wait for the Admin Approval",
    ...
    return (
        <div>
        <Helmet>
      <title>Momentum Daily | Update Articles</title>
     
    </Helmet>
        <h1 className="text-3xl font-semibold text-center text-[#284b63] my-4">Update Your Articles Here </h1>
      {/* add articles form  */}
        <div className="my-4">
        ...
        <div className="form-control w-full ">
              ...
              <input {...register("title" , {required : true})} type="text" defaultValue={newUpdatedInfo?.title} required placeholder="Title" className="input input-bordered w-full" />
              ...
              <input {...register("shortdescription" , {required : true})} defaultValue={newUpdatedInfo?.shortdescription} type="text" required placeholder="Write Down Short Description" className="input input-bordered w-full" />
            ...
            <textarea {...register("description" , {required : true})} defaultValue={newUpdatedInfo?.description} className="textarea textarea-bordered h-24" placeholder="Write Down Full Description"></textarea>
    ...
    className="select select-bordered w-full max-w-md">
      ...
      <option value="BBC">BBC</option>
          ...
          defaultValue={null}
           render={({field}) => (
            ...
            {/* react select */}
              ...
              <input {...register("author" , {required : true})} defaultValue={newUpdatedInfo?.author} type="text" required placeholder="Author Name" className="input input-bordered w-full " />
         ...
         {/* author & types */}
          ...
          <button type="submit" className="px-10 py-3 bg-[#3c6e71] text-white font-medium text-xl flex items-center gap-4">Update Articles <LuUploadCloud /></button>
          </div>
        </form>
        </div>
         {/* add articles form  */}
